 ...
 * - /brokers/topics/{topic}/partitions/{partition_id}/state: 记录分区 leader 副本所在的 brokerId、leaderEpoch、ISR 集合，以及 zkVersion 等
 * - /controller: 记录当前 controller leader 的 ID
 * - /controller_epoch: 记录当前 controller leader 的年代信息
 * - /admin/reassign_partitions: 记录需要进行副本重新分配的分区
...
object ZkUtils {
    ...
    val SecureZkRootPaths: Seq[String] = Seq(AdminPath,
        BrokersPath,
        ClusterPath,
        ConfigPath,
        ControllerPath,
    ...
    val SensitiveZkRootPaths: Seq[String] = Seq(ConfigUsersPath)
    ...
    def sensitivePath(path: String): Boolean = {
        path != null && SensitiveZkRootPaths.exists(path.startsWith)
    }
    ...
    def getEntityConfigPath(entityType: String, entity: String): String = getEntityConfigRootPath(entityType) + "/" + entity

    def getEntityConfigPath(entityPath: String): String = ZkUtils.ConfigPath + "/" + entityPath
...
class ZkUtils(val zkClient: ZkClient,
    ...
    val persistentZkPaths: Seq[String] = Seq(
        ...
        ConfigChangesPath, // config/changes
        getEntityConfigRootPath(ConfigType.Topic), // /config/topics
        getEntityConfigRootPath(ConfigType.Client), // /config/clients
    ...
    object ClusterId {
        ...
        def fromJson(clusterIdJson: String): String = {
            Json.parseFull(clusterIdJson).map { m =>
                ...
                clusterIdMap("id").asInstanceOf[String]
            }.getOrElse(throw new KafkaException(s"Failed to parse the cluster id json $clusterIdJson"))
        }
    }

    /**
     * 读取 /cluster/id 路径，获取 clusterId
     *
     * {"version":"1","id":"QzFnOeZoRnyWEzu-ybGqDQ"}
     *
     * @return
     */
    def getClusterId: Option[String] = readDataMaybeNull(ClusterIdPath)._1.map(ClusterId.fromJson)
    ...
    def getAllBrokersInCluster: Seq[Broker] = {
        ...
        brokerIds.map(_.toInt).map(getBrokerInfo).filter(_.isDefined).map(_.get)
    }
        ...
        leaderAndIsrOpt match {
            case Some(leaderAndIsr) =>
                Json.parseFull(leaderAndIsr) match {
                    ...
                    case Some(m) => m.asInstanceOf[Map[String, Any]]("leader_epoch").asInstanceOf[Int]
                }
    ...
    def getInSyncReplicasForPartition(topic: String, partition: Int): Seq[Int] = {
        ...
        leaderAndIsrOpt match {
            case Some(leaderAndIsr) =>
                Json.parseFull(leaderAndIsr) match {
                    case Some(m) => m.asInstanceOf[Map[String, Any]]("isr").asInstanceOf[Seq[Int]]
     ...
     */
    def updatePersistentPath(path: String, data: String, acls: java.util.List[ACL] = UseDefaultAcls): Unit = {
        val acl = if (acls eq UseDefaultAcls) ZkUtils.defaultAcls(isSecure, path) else acls
        try {
    ...
    def getConsumerGroups: Seq[String] = {
        getChildren(ConsumersPath)
    }

    def getTopicsByConsumerGroup(consumerGroup: String): Seq[String] = {
...
class ZKGroupDirs(val group: String) {
    def consumerDir: String = ConsumersPath

    def consumerGroupDir: String = consumerDir + "/" + group

    def consumerRegistryDir: String = consumerGroupDir + "/ids"

    def consumerGroupOffsetsDir: String = consumerGroupDir + "/offsets"

    def consumerGroupOwnersDir: String = consumerGroupDir + "/owners"
}

class ZKGroupTopicDirs(group: String, topic: String) extends ZKGroupDirs(group) {
    def consumerOffsetDir: String = consumerGroupOffsetsDir + "/" + topic

    def consumerOwnerDir: String = consumerGroupOwnersDir + "/" + topic
}
...
class ZKConfig(props: VerifiableProperties) {
    ...
    /** ZK host string */
    val zkConnect: String = props.getString(ZkConnectProp)

    /** zookeeper session timeout */
    val zkSessionTimeoutMs: Int = props.getInt(ZkSessionTimeoutMsProp, 6000)

    /** the max time that the client waits to establish a connection to zookeeper */
    val zkConnectionTimeoutMs: Int = props.getInt(ZkConnectionTimeoutMsProp, zkSessionTimeoutMs)

    /** how far a ZK follower can be behind a ZK leader */
    val zkSyncTimeMs: Int = props.getInt(ZkSyncTimeMsProp, 2000)
}
...
class ZKCheckedEphemeral(path: String,
    ...
    val defaultAcls: util.List[ACL] = ZkUtils.defaultAcls(isSecure, path)
