...
case class LogConfig(props: java.util.Map[_, _]) extends AbstractConfig(LogConfig.configDef, props, false) {
     ...
     */
    val segmentSize: Integer = getInt(LogConfig.SegmentBytesProp)
    val segmentMs: lang.Long = getLong(LogConfig.SegmentMsProp)
    val segmentJitterMs: lang.Long = getLong(LogConfig.SegmentJitterMsProp)
    val maxIndexSize: Integer = getInt(LogConfig.SegmentIndexBytesProp)
    val flushInterval: lang.Long = getLong(LogConfig.FlushMessagesProp)
    val flushMs: lang.Long = getLong(LogConfig.FlushMsProp)
    val retentionSize: lang.Long = getLong(LogConfig.RetentionBytesProp)
    val retentionMs: lang.Long = getLong(LogConfig.RetentionMsProp)
    val maxMessageSize: Integer = getInt(LogConfig.MaxMessageBytesProp)
    val indexInterval: Integer = getInt(LogConfig.IndexIntervalBytesProp)
    val fileDeleteDelayMs: lang.Long = getLong(LogConfig.FileDeleteDelayMsProp)
    val deleteRetentionMs: lang.Long = getLong(LogConfig.DeleteRetentionMsProp)
    val compactionLagMs: lang.Long = getLong(LogConfig.MinCompactionLagMsProp)
    val minCleanableRatio: lang.Double = getDouble(LogConfig.MinCleanableDirtyRatioProp)
    val compact: Boolean = getList(LogConfig.CleanupPolicyProp).asScala.map(_.toLowerCase(Locale.ROOT)).contains(LogConfig.Compact)
    val delete: Boolean = getList(LogConfig.CleanupPolicyProp).asScala.map(_.toLowerCase(Locale.ROOT)).contains(LogConfig.Delete)
    val uncleanLeaderElectionEnable: lang.Boolean = getBoolean(LogConfig.UncleanLeaderElectionEnableProp)
    val minInSyncReplicas: Integer = getInt(LogConfig.MinInSyncReplicasProp)
    val compressionType: String = getString(LogConfig.CompressionTypeProp).toLowerCase(Locale.ROOT)
    val preallocate: lang.Boolean = getBoolean(LogConfig.PreAllocateEnableProp)
    val messageFormatVersion = ApiVersion(getString(LogConfig.MessageFormatVersionProp))
    val messageTimestampType: TimestampType = TimestampType.forName(getString(LogConfig.MessageTimestampTypeProp))
    val messageTimestampDifferenceMaxMs: Long = getLong(LogConfig.MessageTimestampDifferenceMaxMsProp).longValue
    val LeaderReplicationThrottledReplicas: util.List[String] = getList(LogConfig.LeaderReplicationThrottledReplicasProp)
    val FollowerReplicationThrottledReplicas: util.List[String] = getList(LogConfig.FollowerReplicationThrottledReplicasProp)
