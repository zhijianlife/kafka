...
class PartitionTopicInfo(val topic: String,
    ...
    def getConsumeOffset(): Long = consumedOffset.get

    def getFetchOffset(): Long = fetchedOffset.get

    def resetConsumeOffset(newConsumeOffset: Long): Unit = {
        consumedOffset.set(newConsumeOffset)
        debug("reset consume offset of " + this + " to " + newConsumeOffset)
    }

    def resetFetchOffset(newFetchOffset: Long): Unit = {
    ...
    def enqueue(messages: ByteBufferMessageSet) {
        ...
        if (size > 0) {
            ...
            chunkQueue.put(FetchedDataChunk(messages, this, fetchedOffset.get))
...
object PartitionTopicInfo {
    ...
    def isOffsetInvalid(offset: Long): Boolean = offset < 0L
}